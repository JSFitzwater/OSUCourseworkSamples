...
const char *			CL_FILE_NAME = { "reduct.cl" };
const float			TOL = 0.0001f;

void				Wait( cl_command_queue, int );
...
main( int argc, char *argv[ ] )
{
    ...
    for(int xxx = 0; xxx < 3; xxx++)
    {
        ...
        if( status != CL_SUCCESS ){ fprintf( stderr, "clEnqueueWriteBuffer failed (1)\n" ); }

        status = clEnqueueWriteBuffer( cmdQueue, dB, CL_FALSE, 0, abSize, hB, 0, NULL, NULL );
        if( status != CL_SUCCESS ){ fprintf( stderr, "clEnqueueWriteBuffer failed (2)\n" ); }
        ...
        if( status != CL_SUCCESS ){ fprintf( stderr, "clEnqueueNDRangeKernel failed: %d\n", status ); }
		...
		float hSum = 0.;
		for( int yy = 0; yy < NUM_WORK_GROUPS; yy++ )
		{
			hSum += hC[ yy ];
		}
        ...
        gmps += (double)NUM_ELEMENTS/(time1-time0)/1000000000.0;
    }


	fprintf( stderr, "%8d,%4d,%10d,%10.3lf\n",
		NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, (gmps/3));
...
void
Wait( cl_command_queue queue, int location )
{
      cl_event wait;
      cl_int      status;
      ...
      if( status != CL_SUCCESS )
	      fprintf( stderr, "Wait %d: clEnqueueMarker failed\n", location );
      if( status == CL_INVALID_COMMAND_QUEUE )
	      fprintf( stderr, "Wait %d: clEnqueueMarker  - invalid cmd queue\n", location );
      if( status == CL_INVALID_VALUE )
	      fprintf( stderr, "Wait %d: clEnqueueMarker  - invalid value\n", location );
      if( status == CL_OUT_OF_RESOURCES )
	      fprintf( stderr, "Wait %d: clEnqueueMarker  - out of resources\n", location );
      if( status == CL_OUT_OF_HOST_MEMORY )
	      fprintf( stderr, "Wait %d: clEnqueueMarker  - out of host memory\n", location );

      status = clWaitForEvents( 1, &wait );
      if( status != CL_SUCCESS )
	      fprintf( stderr, "Wait %d: clWaitForEvents failed\n", location );
      if( status == CL_INVALID_COMMAND_QUEUE )
	      fprintf( stderr, "Wait %d: clWaitForEvents  - invalid cmd queue\n", location );
      if( status == CL_INVALID_CONTEXT )
	      fprintf( stderr, "Wait %d: clWaitForEvents  - invalid context\n", location );
      if( status == CL_INVALID_VALUE )
	      fprintf( stderr, "Wait %d: clWaitForEvents  - invalid value\n", location );
      if( status == CL_INVALID_EVENT )
	      fprintf( stderr, "Wait %d: clWaitForEvents  - invalid event\n", location );
      if( status == CL_OUT_OF_RESOURCES )
	      fprintf( stderr, "Wait %d: clWaitForEvents  - out of resources\n", location );
      if( status == CL_OUT_OF_HOST_MEMORY )
	      fprintf( stderr, "Wait %d: clWaitForEvents  - out of host memory\n", location );
}
