...
@stock_data_api.route('/stock-data/latest')
def stock_data_latest():
...
@stock_data_api.route('/stock-data/<symbol>/<date>')
def stock_data_symbol_date(symbol, date):
  # Get the data for the specified date
  stock_data_query = (StockData.query
    .filter(StockData.symbol == symbol, StockData.date == date)
    .first())
  stock_data_schema = StockDataSchema(many=False, exclude=(['symbol', 'id']))
  stock_data_dump = stock_data_schema.dump(stock_data_query).data
