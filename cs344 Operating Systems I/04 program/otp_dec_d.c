...
/* W. Gay, Warren: Chapter 14 code: mkaddr.c */
/**  Resources:
 **
...
int main(int argc, char *argv[]) 
{
    ...
    if(bindTry < 0 ){
		// fprintf(stderr,"OTP_DEC_D: ERROR binding\n"); 
        exit(1);
	}
     ...
     */
        ...
        sizeClieInfo = sizeof(clieAddr);
            ...
            if((childPID = fork()) == 0)
            {
                ...
                //  recv ID from client
                // printf("OTP_DEC_D: recvd from client: \"%s\"\n",buffer);

                // security: audit client ID
                if(strcmp("otp_dec",buffer) != 0){
                    charsScribe = send( estabConnectFD, "otp_dec_d", 9, 0);
                    if(charsScribe < 0 ){
                        // fprintf(stderr,"OTP_DEC_D: ERROR writing to socket\n"); 
                        exit(1);
                    }
                    printf("OTP_DEC_D: prohibited client\n");
                   ...
                   //                     strlen(buffer), 0 );
                    ...
                    if(charsScribe < strlen(buffer) ){
                        int foo2=0; // printf("OTP_DEC_D: WARNING, Not all data written to socket!\n");
                    }
